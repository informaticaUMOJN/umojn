...
<?php }
	else
	{
		...
		$mbPermisoUsuario = fxPermisoUsuario("procPlanPosgrado", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
				...
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true">
							<thead>
								<tr>
									<th data-column-id="PLANPOSGRADO_REL" data-identifier="true" data-width="15%" data-align="left">Plan de posgrado</th>
									<th data-column-id="NOMBRE_040" data-width="55%" data-align="left">Carrera</th>
									<th data-column-id="PERIODO_230" data-align="left">Período</th>
									<th data-column-id="ACTIVO_230" data-header-align="center" data-width="10%" data-align="center">Activo</th>
								</tr>
							</thead>
							<tbody>
							<?php
								$mDatos = fxDevuelvePlanPosgrado(1);
								...
								while ($mFila = $mDatos->fetch())
								{
									...
									echo ("<td>" . $mFila["PERIODO_230"] . "</td>");

									$mbActivo = intval($mFila["ACTIVO_230"]);
									if ($mbActivo == 0)
										$msActivo = "";
									else
										$msActivo = "X";
...
<script>
        $(function() {
			...
			$("#append").on("click", function() {
                $.redirect("procPlanPosgrado.php", "POST");
			});
      			
            $("#edit").on("click", function() {
				if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
                {
                    var codPlanPosgrado = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("procPlanPosgrado.php", {UMOJN: codPlanPosgrado}, "POST");
                }
            });
