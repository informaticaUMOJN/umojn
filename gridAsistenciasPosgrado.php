...
<?php }
	else
	{
		...
		$mbPermisoUsuario = fxPermisoUsuario("procAsistenciaPos", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
		...
		if (isset($_POST["UMOJN"]))
            {
                fxBorrarAsistenciaPos($_POST["UMOJN"]);
				fxAgregarBitacora($_SESSION["gsUsuario"], "UMO300A", $_POST["UMOJN"], "", "Borrar", "");
            }
		?>
    	<div class="container">
        	<div id="DivContenido">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true">
							<thead>
								<tr>
									<th data-column-id="ASISTENCIAPOS_REL" data-identifier="true" data-width="15%" data-align="left">Asistencia</th>
									<th data-column-id="NOMBRE_100" data-width="20%" data-align="left">Docente</th>
									<th data-column-id="NOMBRE_240" data-width="20%" data-align="left">Curso</th>
									<th data-column-id="NOMBRE_040" data-width="15%" data-align="left">Carrera</th>
									<th data-column-id="FECHA_300" data-width="15%" data-align="left">Fecha</th>
									<th data-column-id="COHORTE_300" data-width="15%" data-align="left">Cohorte</th>
								</tr>
							</thead>
							<tbody>
							<?php
								if ($mbAdministrador == 1 or $mbSupervisor == 1)
									$mDatos = fxDevuelveAsistenciaPos(1, "", "");
								else
									$mDatos = fxDevuelveAsistenciaPos(1, $msDocente, "");
								
								while ($mFila = $mDatos->fetch())
								{
									echo ("<tr>");
									echo ("<td>" . $mFila["ASISTENCIAPOS_REL"] . "</td>");
									echo ("<td>" . $mFila["NOMBRE_100"] . "</td>");
									echo ("<td>" . $mFila["NOMBRE_240"] . "</td>");
									echo ("<td>" . $mFila["NOMBRE_040"] . "</td>");
									$fecha = date_create_from_format('Y-m-d', $mFila["FECHA_300"]);
									echo ("<td>" . date_format($fecha, 'd-m-Y') . "</td>");
									echo ("<td>" . $mFila["COHORTE_300"] . "</td>");
									echo ("</tr>");
								}
							}
							?>
...
<script>
        $(function() {
			...
			$("#append").on("click", function() {
                $.redirect("procAsistenciaPos.php", "POST");
			});
			...
			$("#remove").on("click", function() {
                if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
                {
                    ...
                    $.redirect("gridAsistenciasPosgrado.php", {UMOJN: codAsistencia}, "POST");
                }
			});
            ...
            $("#edit").on("click", function() {
                ...
                {
                    var codAsistencia = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("procAsistenciaPos.php", {UMOJN: codAsistencia}, "POST");
                }
            });
