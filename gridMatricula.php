...
<?php }
	else
	{
		...
		$mbPermisoUsuario = fxPermisoUsuario("procMatricula", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
        	...
        	<div id="DivContenido">
				<div id="lateral">
					<?php
						...
						echo('<label id="imprimir" data-toggle="tooltip" data-placement="top" title="Hoja de matrícula"><img src="imagenes/btnLateralImprimir.png" height="80%" style="cursor:pointer" /></label>');
					?>
				</div>
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true" style="font-size:small">
							...
							<?php
								...
								while ($mFila = $mDatos->fetch())
								{
									...
									$fecha = date_create_from_format('Y-m-d', $mFila["FECHA_030"]);
									echo ("<td>" . date_format($fecha, 'd-m-Y') . "</td>");
									if ($mFila["ESTADO_030"]==0)
										echo ("<td>Activo</td>");
									else{
										if ($mFila["ESTADO_030"]==1)
											echo ("<td>Inactivo</td>");
										else
...
<script>
	$(function() {
		...
		$("#print").on("click", function() {
			...
			{
				var msCodigo = $.trim($("#grid").bootgrid("getSelectedRows"));
				$.redirect("repHojaMatricula.php", {UMOJN: msCodigo}, "POST", "_blank");
			}
		});
