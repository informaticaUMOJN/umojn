...
/**********Verificar la existencia de la asistencia**********/
if (isset($_POST["curso2"]) and isset($_POST["cohorte2"]) and isset($_POST["fecha"]))
{
    $m_cnx_MySQL = fxAbrirConexion();
    $msCurso = $_POST["curso2"];
    $msCohorte = $_POST["cohorte2"];
    $msFecha = $_POST["fecha"];
    $msConsulta = "select * from UMO300A where FECHA_300 = ? and CURSO_REL = ? and COHORTE_300 = ?";
    $mDatos = $m_cnx_MySQL->prepare($msConsulta);
