<?php
	function fxGuardarExpDigital($msFecha, $msDefensa, $msCodCarrera, $msCarrera)
	{
		...
		$msConsulta = "insert into UMO001B (EXPDIGITAL_REL, CARRERA_REL, FECHA_001, FECHADEFENSA_001, CARRERA_001) values (?, ?, ?, ?, ?)";
		$mDatos = $m_cnx_MySQL->prepare($msConsulta);
		$mDatos->execute([$msCodigo, $msCodCarrera, $msFecha, $msDefensa, $msCarrera]);
		return $msCodigo;
	}

	function fxModificarExpDigital($msCodigo, $msCodCarrera, $msFecha, $msDefensa, $msCarrera)
	{
		$m_cnx_MySQL = fxAbrirConexion();
		$msConsulta = "update UMO001B set FECHA_001 = ?, FECHADEFENSA_001 = ?, CARRERA_REL = ?, CARRERA_001 = ? where EXPDIGITAL_REL = ?";
		$mDatos = $m_cnx_MySQL->prepare($msConsulta);
		$mDatos->execute([$msFecha, $msDefensa, $msCodCarrera, $msCarrera, $msCodigo]);
	}
	...
	function fxDevuelveExpDigital($mbLlenaGrid, $msCodigo = "")
	{
		...
		{
			$msConsulta = "select EXPDIGITAL_REL, CARRERA_REL, FECHA_001, FECHADEFENSA_001, CARRERA_001 FROM UMO001B where EXPDIGITAL_REL = ?";
			$mDatos = $m_cnx_MySQL->prepare($msConsulta);
			$mDatos->execute([$msCodigo]);
		}
		return $mDatos;
	...
	{
		$m_cnx_MySQL = fxAbrirConexion();
		$msConsulta = "select EXPDIGITAL_REL, CARNET_REL, FOLDER_002, NOMBRE_002, REGISTRO_002, TOMO_002, FOLIO_002 FROM UMO002B where EXPDIGITAL_REL = ? order by REGISTRO_002";
		$mDatos = $m_cnx_MySQL->prepare($msConsulta);
		$mDatos->execute([$msCodigo]);
