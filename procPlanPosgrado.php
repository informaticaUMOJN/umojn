<?php
    ...
    require_once ("funciones/fxPlanPosgrado.php");
...
<?php }
	else
	{
		...
		$mbPermisoUsuario = fxPermisoUsuario("procPlanPosgrado");
		...
		{
			if (isset($_POST["txtPlanPosgrado"]))
			{
				$msCodigo = $_POST["txtPlanPosgrado"];
                ...
                if ($msCodigo == "")
                {
                    $msCodigo = fxGuardarPlanPosgrado($msCarrera, $msPeriodo, $msGrado, $mnHoras, $mnCreditos, $mnTurno, $mnRegimen, $mnModalidad, $mbActivo);
                    $msBitacora = $msCodigo . "; " . $msCarrera . "; " . $msPeriodo . "; " . $msGrado . "; " . $mnHoras . "; " . $mnCreditos . "; " . $mnTurno . "; " . $mnRegimen . "; " . $mnModalidad . "; " . $mbActivo;
                    fxAgregarBitacora ($_SESSION["gsUsuario"], "KDSA230A", $msCodigo, "", "Agregar", $msBitacora);
                }
                else
                {
                    fxModificarPlanPosgrado($msCodigo, $msCarrera, $msPeriodo, $msGrado, $mnHoras, $mnCreditos, $mnTurno, $mnRegimen, $mnModalidad, $mbActivo);
                    fxBorrarDetPlanPosgrado($msCodigo);
                    $msBitacora = $msCodigo . "; " . $msCarrera . "; " . $msPeriodo . "; " . $msGrado . "; " . $mnHoras . "; " . $mnCreditos . "; " . $mnTurno . "; " . $mnRegimen . "; " . $mnModalidad . "; " . $mbActivo;
                    fxAgregarBitacora ($_SESSION["gsUsuario"], "KDSA230A", $msCodigo, "", "Modificar", $msBitacora);
                }
				...
				?>
<meta http-equiv="Refresh" content="0;url=gridPlanPosgrado.php" /><?php
			}
			else
			{
                ...
                if ($msCodigo != "")
                {
                    $mRecordSet = fxDevuelvePlanPosgrado(0, $msCodigo);
                    $mFila = $mRecordSet->fetch();
                    $msCarrera = $mFila["CARRERA_REL"];
                    $msPeriodo = $mFila["PERIODO_230"];
                    $msGrado = $mFila["GRADO_230"];
                    $mnHoras = $mFila["HORAS_230"];
                    $mnCreditos = $mFila["CREDITOS_230"];
                    $mnTurno = $mFila["TURNO_230"];
                    $mnRegimen = $mFila["REGIMEN_230"];
                    $mnModalidad = $mFila["MODALIDAD_230"];
                    $mbActivo = $mFila["ACTIVO_230"];
                }
                else
                {
                    $msCarrera = "";
    ...
    <div id="DivContenido">
        <div class="row">
            <div class="col-xs-12 col-md-12">
                <form id="procPlanPosgrado" name="procPlanPosgrado">
                    <div class="row">
                        <div class="col-md-12">
                            <input type="submit" id="Guardar" name="Guardar" value="Guardar" class="btn btn-primary" />
                            <input type="button" id="Cancelar" name="Cancelar" value="Cancelar" class="btn btn-primary"  onclick="location.href='gridPlanPosgrado.php';" />
                        </div>
                    </div>
                    ...
                    <div class="easyui-tabs tabs-narrow" style="width:100%;height:auto">
                        ...
                        <div title="Generales" style="padding:10px">
                            ...
                            <div class="col-sm-auto offset-sm-0 col-md-11 offset-md-1">
                                <div class="form-group row">
                                    <label for="txtPlanPosgrado" class="col-sm-auto col-md-2 col-form-label">Código del Plan</label>
                                    <div class="col-sm-12 col-md-3">
                                        <?php echo('<input type="text" class="form-control" id="txtPlanPosgrado" name="txtPlanPosgrado" value="' . $msCodigo . '" readonly />'); ?>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select class="form-control" id="cboCarrera" name="cboCarrera" onchange="llenaCursos(this.value)">
                                        <?php
                        ...
                        <div title="Detalle de cursos" style="padding:10px">
                                    ...
                                    <div class="col-sm-auto col-md-12">
                                        <div id="dvCUR">
                                            <table id="dgCUR" class="easyui-datagrid table" data-options="iconCls:'icon-edit', toolbar:'#tbCUR', footer:'#ftCUR', singleSelect:true, method:'get', onClickCell: onClickCellCUR">
                                                ...
                                                <tbody>
                                                <?php
                                                    $mDatos = fxObtenerDetPlanPosgrado($msCodigo);
                                                    while ($mFila = $mDatos->fetch())
                                                    {
                                                        echo ("<tr>");
                                                        echo ("<td>" . $mFila["PLANPOSGRADO_REL"] . "</td>");
                                                        echo ("<td>" . $mFila["DETPLAN_REL"] . "</td>");
                                ...
                                <div id="tbCUR" style="height:auto; padding-top:1%; padding-bottom:2%">
                                    <table width="100%">
                                        ...
                                        <tr>
                                            ...
                                            <td width="40%">
                                                <select id="cboCurso" name="cboCurso" class="form-control" style="width:90%">
                                                <?php
                                                    $msConsulta = "select CURSOPOSGRADO_REL, NOMBRE_240 from UMO240A where CARRERA_REL = ? order by NOMBRE_240";
                                                    $mCursos = $m_cnx_MySQL->prepare($msConsulta);
                                                    $mCursos->execute([$msCarrera]);
                                                    while ($mAuxFila = $mCursos->fetch())
                                                    {
                                                        echo("<option value='" . $mAuxFila["CURSOPOSGRADO_REL"] . "'>" . $mAuxFila["NOMBRE_240"] . "</option>");
                                                    }
                                                ?>
                                                </select>
                                            </td>
                                        ...
                                        <tr>
                                            ...
                                            <td width="40%"><input type="text" id="txtModulo" class="form-control" style="width:90%" value=""></td>
...
{
    var dgCursos = $('#dgCUR');
    dgCursos.datagrid({striped: true});
...
function validaRegistro(){
    var gridCurso = $('#dgCUR').datagrid('getData');
    var curso = document.getElementById("cboCurso").value;
    var registros = $('#dgCUR').datagrid('getRows').length;

    if (document.getElementById("txnPeriodo").value <= 0)
    {
        $.messager.alert('UMOJN','El semestre, cuatrimestre o trimestre está en cero.','warning');
		return false;
    }

    for (i=0; i<registros; i++)
    {
        if (gridCurso.rows[i].codCurso == curso)
        {
            $.messager.alert('UMOJN','El curso ya fue ingresado en este plan de estudios.','warning');
			return false;
        }
    }
...
function llenaCursos (carrera)
{
    ...
    $.ajax({
        url: 'funciones/fxDatosPlanPosgrado.php',
        type: 'post',
        data: datos,
        contentType: false,
        processData: false,
...
function verificarFormulario() {
    ...
    if ($('#dgCUR').datagrid('getRows').length <= 0) {
        $.messager.alert('UMOJN', 'Falta el detalle de los cursos.', 'warning');
        return false;
    }
...
function appendCUR() {
    txCurso = $('select[name="cboCurso"] option:selected').text();

    if (validaRegistro())
    {
        if (endEditingCUR()) {
            $('#dgCUR').datagrid('appendRow', {
                codPlanPosgrado: $('#txtPlanPosgrado').val(),
                codCurso: $('#cboCurso').val(),
                curso: txCurso,
...
$('form').submit(function(e) {
    ...
    if (verificarFormulario() == true) {
        ...
        var registros;
        var i;
        var gridDetalle = $('#dgCUR').datagrid('getData');

        texto = '{"txtPlanPosgrado":"' + document.getElementById("txtPlanPosgrado").value + '", ';
        ...
        if (registros >= 0) {
            texto += '"gridDetalle": [';
            for (i = 0; i <= registros; i++) {
                texto += '{"codPlanPosgrado":"' + gridDetalle.rows[i].codPlanPosgrado + '",';
                texto += '"codConsecutivo":"' + i + '",';
                texto += '"codCurso":"' + gridDetalle.rows[i].codCurso + '",';
        ...
        .done(function() {
            location.href = "gridPlanPosgrado.php";
        })
        .fail(function() {
            console.log('Error')
        });
