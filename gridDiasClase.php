...
<?php }
	else
	{
		...
		$mbPermisoUsuario = fxPermisoUsuario("hrrDiasClase", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
				...
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true">
							<thead>
								<tr>
									<th data-column-id="DIACLASE_REL" data-identifier="true" data-align="left" data-width="10%">Código</th>
									<th data-column-id="NOMBRE_060" data-align="left" data-width="40%">Carrera</th>
									<th data-column-id="NOMBRE_040" data-align="left" data-width="40%">Asignatura</th>
									<th data-column-id="ANNOLECTIVO_080" data-align="left" data-width="10%">Año</th>
								</tr>
							</thead>
							<tbody>
							<?php
								$mDatos = fxDevuelveDiasClase(1);
								...
								while ($mFila = $mDatos->fetch())
								{
									...
									echo ("<td>" . $mFila["ANNOLECTIVO_080"] . "</td>");
									echo ("</tr>");
								}
							}
							?>
...
<script>
        $(function() {
			...
			$("#append").on("click", function() {
                $.redirect("hrrDiasClase.php", "POST");
			});
  
			$("#remove").on("click", function() {
                if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
                {
                    var codDiaClase = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("gridDiasClase.php", {UMOJN: codDiaClase}, "POST");
                }
			});
            ...
            $("#edit").on("click", function() {
                ...
                {
                    var codDiaClase = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("hrrDiasClase.php", {UMOJN: codDiaClase}, "POST");
                }
            });
