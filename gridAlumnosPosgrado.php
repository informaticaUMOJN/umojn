...
<?php }
	else
	{
		...
		$mbPermisoUsuario = fxPermisoUsuario("catAlumnosPosgrado", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
				...
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true">
							<thead>
								<tr>
									<th data-column-id="ESTUDIANTEPOS_REL" data-identifier="true" data-align="left" data-width="12%">Estudiante</th>
									<th data-column-id="NOMBRECOMPLETO" data-header-align="left" data-width="32%">Nombre completo</th>
									<th data-column-id="CELULAR_250" data-align="left" data-header-align="left" data-width="15%">Celular</th>
									<th data-column-id="CORREOI_250" data-align="left" data-header-align="left" data-width="35%">eMail</th>
								</tr>
							</thead>
							<tbody>
							<?php
								$mDatos = fxDevuelveEstudiantePos(1);
								while ($mFila = $mDatos->fetch())
								{
									echo ("<tr>");
									echo ("<td>" . $mFila["ESTUDIANTEPOS_REL"] . "</td>");

									if (trim($mFila["APELLIDO2_250"])!="")
										$msNombre = trim($mFila["APELLIDO1_250"]) . " " . $mFila["APELLIDO2_250"] . ", ";
									else
										$msNombre = trim($mFila["APELLIDO1_250"]) . ", ";

									if (trim($mFila["NOMBRE2_250"])!="")
										$msNombre .= trim($mFila["NOMBRE1_250"]) . " " . trim($mFila["NOMBRE2_250"]);
									else
										$msNombre .= trim($mFila["NOMBRE1_250"]);

									echo ("<td>" . $msNombre . " " . "</td>");
									echo ("<td>" . $mFila["CELULAR_250"] . "</td>");
									echo ("<td>" . $mFila["CORREOI_250"] . "</td>");
								}
							}
							?>
							</tbody>
...
<script>
	...
	$(function() {
		...
		$("#append").on("click", function() {
			$.redirect("catAlumnosPosgrado.php", "POST");
		});
		...
		$("#edit").on("click", function() {
			if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
			{
				...
				$.redirect("catAlumnosPosgrado.php", {UMOJN: codEstudiante}, "POST");
			}
		});
		...
		$("#matricula").on("click", function() {
			if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
			{
				...
				$.redirect("procMatriculaPos.php", {mAccion: 1, mEstudiante: msEstudiante}, "POST");
			}
		});
