...
<?php }
	else
	{
		...
		if (isset($_POST["UMOJN"]))
            {
                fxBorrarCursoPosgrado($_POST["UMOJN"]);
				...
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true" style="font-size:small">
							<thead>
								<tr>
									<th data-column-id="CURSOPOSGRADO_REL" data-identifier="true" data-align="left" data-width="10%">Curso</th>
									<th data-column-id="NOMBRE_040" data-align="left" data-width="40%">Carrera</th>
									<th data-column-id="NOMBRE_240" data-align="left" data-width="40%">Nombre</th>
									<th data-column-id="ACTIVO_240" data-align="center" data-width="10%" data-header-align="center">Activo</th>
								</tr>
							</thead>
							<tbody>
							<?php
								$mDatos = fxDevuelveCursoPosgrado(1);
...
<script>
        $(function() {
			...
			$("#append").on("click", function() {
                $.redirect("catCursosPosgrado.php", "POST");
			});
  
			$("#remove").on("click", function() {
                if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
                {
                    var codCursosPosgrado = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("gridCursosPosgrado.php", {UMOJN: codCursosPosgrado}, "POST");
                }
			});
            ...
            $("#edit").on("click", function() {
                ...
                {
                    var codCursosPosgrado = $.trim($("#grid").bootgrid("getSelectedRows"));
                    $.redirect("catCursosPosgrado.php", {UMOJN: codCursosPosgrado}, "POST");
                }
            });
