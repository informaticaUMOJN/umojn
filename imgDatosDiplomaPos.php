...
if (isset($_POST["UMOJN"]))
{
    ...
    while($mFila = $mDatos->fetch())
    {
        ...
        if ($mnRegistros > 0)
        {
            ...
            $msEstudiante = $mrEstudiante["ESTUDIANTEPOS_REL"];
            $msNombre = trim($mrEstudiante["NOMBRE1_250"]);
            if (trim($mrEstudiante["NOMBRE2_250"]) != "")
                $msNombre .= " " . trim($mrEstudiante["NOMBRE2_250"]);
            $msNombre .= " " . trim($mrEstudiante["APELLIDO1_250"]);
            if (trim($mrEstudiante["APELLIDO2_250"]) != "")
                $msNombre .= " " . trim($mrEstudiante["APELLIDO2_250"]);
            ...
            $msConsulta = "select ARCHIVO_251 from UMO251A where ESTUDIANTEPOS_REL = ? and TIPO_REL = 9";
