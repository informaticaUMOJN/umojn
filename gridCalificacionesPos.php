...
<?php }
	else
	{
		...
		$mbPermisoUsuario = fxPermisoUsuario("procCalificacionPos", $mbAgregar, $mbModificar, $mbBorrar, $mbAnular);
		...
		if (isset($_POST["UMOJN"]))
            {
                fxBorrarCalificacionPos($_POST["UMOJN"]);
				fxAgregarBitacora($_SESSION["gsUsuario"], "UMO310A", $_POST["UMOJN"], "", "Borrar", "");
            }
		?>
    	<div class="container">
        	<div id="DivContenido">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<table id="grid" class="table table-condensed table-hover table-striped" data-selection="true" data-multi-select="false" data-row-select="true" data-keep-selection="true" style="font-size:small">
							<thead>
								<tr>
									<th data-column-id="CALIFICACIONPOS_REL" data-identifier="true" data-width="15%" data-align="left">Calificación</th>
									<th data-column-id="NOMBRE_100" data-width="30%" data-align="left">Docente</th>
									<th data-column-id="NOMBRE_240" data-width="35%" data-align="left">Curso</th>
									<th data-column-id="COHORTE_310" data-width="10%" data-align="left">Cohorte</th>
									<th data-column-id="TURNO_310" data-width="10%" data-align="center">Turno</th>
								</tr>
							</thead>
							<tbody>
							<?php
								if ($mbAdministrador == 1 or $mbSupervisor == 1)
									$mDatos = fxDevuelveCalificacionPos(1, "", "");
								else
									$mDatos = fxDevuelveCalificacionPos(1, $msDocente, "");
								
								while ($mFila = $mDatos->fetch())
								{
									echo ("<tr>");
									echo ("<td>" . $mFila["CALIFICACIONPOS_REL"] . "</td>");
									echo ("<td>" . $mFila["NOMBRE_100"] . "</td>");
									echo ("<td>" . $mFila["NOMBRE_240"] . "</td>");
									echo ("<td>" . $mFila["COHORTE_310"] . "</td>");

									switch(intval($mFila["TURNO_310"]))
									{
										case 1:
											$msTurno = 'Diurno';
											break;
...
<script>
	$(function() {
		...
		$("#append").on("click", function() {
			$.redirect("procCalificacionPos.php", "POST");
		});
		...
		$("#remove").on("click", function() {
			if ($.trim($("#grid").bootgrid("getSelectedRows")) != "")
			{
				...
				$.redirect("gridCalificacionesPos.php", {UMOJN: codCalificacion}, "POST");
			}
		});
		...
		$("#edit").on("click", function() {
			...
			{
				var codCalificacion = $.trim($("#grid").bootgrid("getSelectedRows"));
				$.redirect("procCalificacionPos.php", {UMOJN: codCalificacion}, "POST");
			}
		});
